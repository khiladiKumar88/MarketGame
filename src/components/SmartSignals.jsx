    ...
    <div style={{maxWidth:520,margin:'0 auto',padding:'40px 0'}}>
      <div style={{textAlign:'center',marginBottom:32}}>
        ...
        <div style={{fontSize:13,color:'var(--text-muted)',lineHeight:1.8}}>
          AI scans all stocks · Detects candlestick patterns + 7 indicators<br/>
          <strong style={{color:'var(--gold)'}}>Click any signal to validate it with historical backtest</strong><br/>
          Combined score = AI Confidence + Backtest Win Rate
        </div>
      </div>
      <div style={{background:'var(--bg-secondary)',border:'1px solid var(--border)',borderRadius:14,padding:22,marginBottom:14}}>
        <div style={{fontSize:11,fontWeight:700,color:'var(--text-muted)',textTransform:'uppercase',letterSpacing:1,marginBottom:14}}>💰 Your Trading Capital</div>
        <div style={{position:'relative',marginBottom:12}}>
          <span style={{position:'absolute',left:14,top:'50%',transform:'translateY(-50%)',fontSize:22,fontWeight:700,color:'var(--gold)'}}>₹</span>
      ...
      <div style={{background:'rgba(99,102,241,0.06)',border:'1px solid rgba(99,102,241,0.2)',borderRadius:12,padding:14,marginBottom:22}}>
        <div style={{fontSize:10,fontWeight:700,color:'#a78bfa',textTransform:'uppercase',letterSpacing:1,marginBottom:8}}>What Smart Signals does</div>
        <div style={{display:'flex',flexDirection:'column',gap:6}}>
          {[
            '🤖 Scans with 7 indicators: RSI · MACD · Bollinger · ADX · Supertrend · VWAP · Volume Spike',
            '🕯️ Detects up to 16 candlestick patterns with 1–5 star strength rating',
            '🧪 Backtest button: tests signal on ALL historical candles',
            '📊 Combined Score = AI confidence (50%) + backtest win rate (50%)',
            '📋 Shows last 10 times this exact signal fired + what happened each time',
          ].map((t,i)=>(
            <div key={i} style={{fontSize:12,color:'var(--text-secondary)',lineHeight:1.6}}>{t}</div>
          ))}
        </div>
      </div>
  ...
  if (phase==='scanning') return (
    <div style={{maxWidth:460,margin:'60px auto',textAlign:'center'}}>
      ...
      <div style={{fontSize:13,color:'var(--text-muted)',marginBottom:28}}>
        7 indicators · 16 candlestick patterns · {config?.minConf}%+ confidence
      </div>
...
//   if (body(c)/rng(c)<0.1)                                           res.push({n:'Doji',t:'neutral',strength:1,desc:'Market indecision — potential reversal incoming'});
//   if (bull(c)&&lw(c)>body(c)*2&&uw(c)<body(c)*0.5)                 res.push({n:'Hammer',t:'bullish',strength:3,desc:'Strong bullish reversal — buyers rejected lower prices'});
//   if (uw(c)>body(c)*2&&lw(c)<body(c)*0.5)                           res.push({n:'Shooting Star',t:'bearish',strength:3,desc:'Strong bearish reversal — sellers rejected higher prices'});
//   if (bear(p1)&&bull(c)&&c.open<p1.close&&c.close>p1.open)          res.push({n:'Bullish Engulfing',t:'bullish',strength:4,desc:'Bulls completely overwhelmed bears — high conviction buy'});
//   if (bull(p1)&&bear(c)&&c.open>p1.close&&c.close<p1.open)          res.push({n:'Bearish Engulfing',t:'bearish',strength:4,desc:'Bears completely overwhelmed bulls — high conviction sell'});
//   if (bear(p2)&&body(p1)<body(p2)*0.3&&bull(c)&&c.close>(p2.open+p2.close)/2) res.push({n:'Morning Star',t:'bullish',strength:5,desc:'Powerful 3-candle reversal — very strong buy signal'});
//   if (bull(p2)&&body(p1)<body(p2)*0.3&&bear(c)&&c.close<(p2.open+p2.close)/2) res.push({n:'Evening Star',t:'bearish',strength:5,desc:'Powerful 3-candle reversal — very strong sell signal'});
//   if (bull(c)&&body(c)/rng(c)>0.85)                                  res.push({n:'Bullish Marubozu',t:'bullish',strength:3,desc:'Strong buying all session — momentum play'});
//   if (bear(c)&&body(c)/rng(c)>0.85)                                  res.push({n:'Bearish Marubozu',t:'bearish',strength:3,desc:'Strong selling all session — momentum short'});
//   if([p2,p1,c].every(bull)&&p1.close>p2.close&&c.close>p1.close)    res.push({n:'3 White Soldiers',t:'bullish',strength:5,desc:'Very strong sustained buying — trend continuation'});
//   if([p2,p1,c].every(bear)&&p1.close<p2.close&&c.close<p1.close)    res.push({n:'3 Black Crows',t:'bearish',strength:5,desc:'Very strong sustained selling — trend continuation'});
//   if(body(c)/rng(c)<0.1&&lw(c)>rng(c)*0.3)                          res.push({n:'Dragonfly Doji',t:'bullish',strength:3,desc:'Bullish doji — buyers strong at lows'});
//   if(body(c)/rng(c)<0.1&&uw(c)>rng(c)*0.3)                          res.push({n:'Gravestone Doji',t:'bearish',strength:3,desc:'Bearish doji — sellers strong at highs'});
//   return res;
// }
...
// // ── Backtest Engine ──────────────────────────────────────────
// function runBacktest(candles, dir) {
//   // Generate signals using same scoring logic on each historical window
//   const trades = [];
//   const minWindow = 40;
...
//     const price  = slice[slice.length-1].close;
//     const pats   = detectPatterns(slice.slice(-3));
//     const sc     = scoreSignal({ r, m, b, a, st, vw, price, pats });

//     // Only record when signal matches current direction and confidence > 65
//     if (sc.dir !== dir || sc.confidence < 65) continue;

//     // Check what happened next 3-5 candles
//     const nextCandles = candles.slice(i+1, i+6);
//     if (!nextCandles.length) continue;
...
//     const highInHold  = Math.max(...nextCandles.map(c=>c.high));
//     const lowInHold   = Math.min(...nextCandles.map(c=>c.low));
//     const pnlPct      = dir==='CALL'
//       ? (exitPrice-entryPrice)/entryPrice*100
//       : (entryPrice-exitPrice)/entryPrice*100;
//     const maxGain     = dir==='CALL'
//       ? (highInHold-entryPrice)/entryPrice*100
//       : (entryPrice-lowInHold)/entryPrice*100;
//     const win         = pnlPct > 0;
...
//   return {
//     trades, last10, winRate, avgWin, avgLoss, avgMaxGain,
//     profitFactor, totalTrades: trades.length,
...
//   const toX=i=>(i/(pts.length-1))*W;
//   const toY=v=>H-((v-min)/range)*(H-4)-2;
//   const d=pts.map((v,i)=>i===0?`M ${toX(i)},${toY(v)}`:`L ${toX(i)},${toY(v)}`).join(' ');
//   const isUp=pts[pts.length-1]>=0;
//   return (
//     <svg width={W} height={H} style={{display:'block'}}>
//       <polyline points={pts.map((v,i)=>`${toX(i)},${toY(v)}`).join(' ')} fill="none" stroke={isUp?'#22c55e':'#ef4444'} strokeWidth={1.5} strokeLinejoin="round"/>
...
//         <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
//           <div>
//             <div style={{fontSize:20,fontWeight:900,color:'var(--text-primary)'}}>{result.sym}
//               <span style={{fontSize:13,color:'var(--text-muted)',fontWeight:400,marginLeft:8}}>Smart Signal Validation</span>
//             </div>
//             <div style={{fontSize:12,color:'var(--text-muted)',fontFamily:'DM Mono',marginTop:2}}>
//               ₹{fmt(result.price)} · {result.chgPct>=0?'+':''}{result.chgPct}% · Vol {result.volSpike}x
//             </div>
...
//         {/* Two columns: Indicators + Patterns */}
//         <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:20}}>
//           {/* Indicators */}
//           <div style={{background:'var(--bg-primary)',borderRadius:12,padding:14}}>
//             <div style={{fontSize:10,fontWeight:700,color:'var(--text-muted)',textTransform:'uppercase',letterSpacing:1,marginBottom:10}}>📊 Indicator Readings</div>
//             {[
//               ['RSI(14)',    result.rsi!=null?`${result.rsi} ${result.rsi<30?'(Oversold 🟢)':result.rsi>70?'(Overbought 🔴)':'(Neutral)'}` :'---', result.rsi<50],
//               ['MACD',      result.macd?`${result.macd.line} (${result.macd.bull?'Bullish ▲':'Bearish ▼'})`:'---', result.macd?.bull],
//               ['Bollinger', result.bb?`${result.bb.pct}% — ${result.bb.pos}`:'---', result.bb?.pct<50],
...
//               ['Supertrend',result.st?.trend??'---', result.st?.trend==='UP'],
//               ['VWAP',      result.vwap?`₹${fmt(result.vwap)} ${result.price>result.vwap?'↑ Above':'↓ Below'}`:'---', result.price>result.vwap],
//               ['Vol Spike', `${result.volSpike}x avg`, result.volSpike>1.5],
//             ].map(([l,v,b])=>(
//               <div key={l} style={{display:'flex',justifyContent:'space-between',padding:'6px 0',borderBottom:'1px solid var(--border)'}}>
//                 <span style={{fontSize:12,color:'var(--text-muted)'}}>{l}</span>
//                 <span style={{fontSize:12,fontFamily:'DM Mono',fontWeight:600,
...
//           <div style={{background:'var(--bg-primary)',borderRadius:12,padding:14}}>
//             <div style={{fontSize:10,fontWeight:700,color:'var(--text-muted)',textTransform:'uppercase',letterSpacing:1,marginBottom:10}}>🕯️ Candlestick Patterns Detected</div>
//             {result.pats?.length ? result.pats.map((p,i)=>(
//               <div key={i} style={{marginBottom:10,padding:'10px 12px',borderRadius:8,
//                 background:p.t==='bullish'?'rgba(34,197,94,0.08)':p.t==='bearish'?'rgba(239,68,68,0.08)':'rgba(201,168,76,0.08)',
//                 border:`1px solid ${p.t==='bullish'?'rgba(34,197,94,0.25)':p.t==='bearish'?'rgba(239,68,68,0.25)':'rgba(201,168,76,0.25)'}`}}>
//                 <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:4}}>
//                   <span style={{fontSize:13,fontWeight:700,color:p.t==='bullish'?'var(--green)':p.t==='bearish'?'var(--red)':'var(--gold)'}}>{p.n}</span>
//                   <div style={{display:'flex',gap:2}}>
//                     {Array.from({length:5},(_, idx)=>(
//                       <div key={idx} style={{width:8,height:8,borderRadius:'50%',
//                         background:idx<p.strength?( p.t==='bullish'?'var(--green)':p.t==='bearish'?'var(--red)':'var(--gold)'):'var(--border)'}}/>
//                     ))}
//                   </div>
//                 </div>
...
//               </div>
//             )) : (
//               <div style={{textAlign:'center',padding:'30px 0',color:'var(--text-muted)',fontSize:13}}>No strong patterns on latest candles</div>
//             )}
//           </div>
//         </div>
...
//             <div style={{fontSize:14,fontWeight:700,color:'var(--gold)'}}>Running Historical Backtest...</div>
//             <div style={{fontSize:12,color:'var(--text-muted)',marginTop:4}}>
//               Testing this signal on {result.candles?.length ?? 0} historical candles
//             </div>
//           </div>
//         )}
...
//               </div>
//               <div style={{fontSize:12,color:'var(--text-muted)',lineHeight:1.8}}>
//                 This signal fired <strong style={{color:'var(--text-primary)'}}>{btData.totalTrades}</strong> times historically.<br/>
//                 Won <strong style={{color:'var(--green)'}}>{btData.wins}</strong> times, lost <strong style={{color:'var(--red)'}}>{btData.losses}</strong> times.<br/>
//                 Average max gain when right: <strong style={{color:'var(--green)'}}>+{btData.avgMaxGain}%</strong>
//               </div>
//             </div>

//             {/* Last 10 historical signals */}
//             <div style={{background:'var(--bg-primary)',borderRadius:10,padding:14}}>
//               <div style={{fontSize:10,fontWeight:700,color:'var(--text-muted)',textTransform:'uppercase',letterSpacing:1,marginBottom:10}}>
//                 📋 Last {btData.last10.length} Times This Signal Fired
//               </div>
...
//                   <div key={i} style={{display:'flex',alignItems:'center',gap:10,padding:'7px 10px',
//                     background:`${t.win?'rgba(34,197,94,0.06)':'rgba(239,68,68,0.06)'}`,
//                     border:`1px solid ${t.win?'rgba(34,197,94,0.15)':'rgba(239,68,68,0.15)'}`,
//                     borderRadius:8}}>
//                     <div style={{width:24,height:24,borderRadius:'50%',flexShrink:0,
//                       display:'flex',alignItems:'center',justifyContent:'center',
//                       background:t.win?'rgba(34,197,94,0.2)':'rgba(239,68,68,0.2)',
//                       fontSize:12}}>{t.win?'✅':'❌'}</div>
//                     <div style={{flex:1}}>
//                       <div style={{fontSize:11,color:'var(--text-muted)'}}>{t.date}</div>
//                       {t.patterns.length>0&&<div style={{fontSize:10,color:'var(--gold)'}}>🕯 {t.patterns[0]}</div>}
//                     </div>
...
//       <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
//         <div style={{fontSize:15,fontWeight:800,color,background:`${color}15`,
...
//         <div style={{fontSize:24,fontWeight:900,color:'var(--text-primary)',marginBottom:8}}>Smart Signal Scanner</div>
//         <div style={{fontSize:13,color:'var(--text-muted)',lineHeight:1.8}}>
//           AI scans all stocks · Detects candlestick patterns + 6 indicators<br/>
//           <strong style={{color:'var(--gold)'}}>Click any signal to validate it with historical backtest</strong><br/>
//           Combined score = AI Confidence + Backtest Win Rate
//         </div>
//       </div>
...
//         <div style={{fontSize:10,fontWeight:700,color:'#a78bfa',textTransform:'uppercase',letterSpacing:1,marginBottom:8}}>What Smart Signals does</div>
//         <div style={{display:'flex',flexDirection:'column',gap:6}}>
//           {['🤖 Scans with 6 indicators: RSI · MACD · Bollinger · ADX · Supertrend · VWAP',
//             '🕯️ Detects 13 candlestick patterns with strength rating',
//             '🧪 Backtest button: tests signal on ALL historical candles',
//             '📊 Combined Score = AI confidence + backtest win rate',
//             '📋 Shows last 10 times this exact signal fired + what happened'].map((t,i)=>(
//             <div key={i} style={{fontSize:12,color:'var(--text-secondary)',lineHeight:1.6}}>{t}</div>
//           ))}
//         </div>
//       </div>
...
// // ── Main Export ───────────────────────────────────────────────
// export default function SmartSignals({ indices, stocks }) {
//   const [phase,    setPhase]    = useState('setup');
//   const [config,   setConfig]   = useState(null);
//   const [progress, setProg]     = useState(0);
//   const [scanMsg,  setScanMsg]  = useState('');
...
//       <div style={{fontSize:18,fontWeight:800,color:'var(--text-primary)',marginBottom:6}}>Smart Scanning...</div>
//       <div style={{fontSize:13,color:'var(--text-muted)',marginBottom:28}}>
//         6 indicators · 13 candlestick patterns · {config?.minConf}%+ confidence
//       </div>
//       <div style={{background:'var(--bg-secondary)',borderRadius:10,height:12,overflow:'hidden',marginBottom:10}}>
//         <div style={{width:`${progress}%`,height:'100%',borderRadius:10,
//           background:'linear-gradient(90deg,#6366f1,#8b5cf6)',transition:'width 0.3s'}}/>
