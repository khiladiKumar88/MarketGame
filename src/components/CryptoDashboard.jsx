...
// ── OVERVIEW COMPONENTS ───────────────────────────────────
// ══════════════════════════════════════════════════════════
// ── SVG Candlestick for Crypto ────────────────────────────
function CryptoSVGCandles({ candles, height = 300, color }) {
  ...
  const candleW  = Math.max(2, Math.floor(gap * 0.7));

  const toY = p  => PAD.top + chartH - ((p - priceMin) / range) * chartH;
  const toX = i  => PAD.left + i * gap + gap / 2;

  const ticks  = Array.from({ length: 6 }, (_, i) => priceMin + (range / 5) * i);
  ...
  function onMove(e) {
    ...
    const idx  = Math.max(0, Math.min(count - 1, Math.round(mx / gap)));
  ...
  const fmtY = v => v >= 10000 ? `$${(v/1000).toFixed(0)}K` : v >= 1000 ? `$${v.toFixed(0)}` : v >= 1 ? `$${v.toFixed(2)}` : `$${v.toFixed(4)}`;
  ...
  return (
    <div style={{ position: 'relative', width: '100%' }}>
      <svg ref={svgRef} viewBox={`0 0 ${W} ${height}`} width="100%" height={height}
        ...
        {ticks.map((t, i) => (
          <g key={i}>
            <line x1={PAD.left} x2={W - PAD.right} y1={toY(t)} y2={toY(t)} stroke="rgba(255,255,255,0.04)" strokeWidth={1} />
            <text x={W - PAD.right + 5} y={toY(t) + 4} fill="#4a5578" fontSize={9} fontFamily="DM Mono">{fmtY(t)}</text>
          </g>
        ))}

        {xLabels.map(({ i, label }) => (
          <text key={i} x={toX(i)} y={height - 4} fill="#4a5578" fontSize={9} fontFamily="DM Mono" textAnchor="middle">{label}</text>
        ))}
        ...
        {candles.map((c, i) => {
          ...
          return (
            <g key={i} opacity={hovered ? 1 : 0.85}>
              {hovered && <rect x={cx - gap / 2} y={PAD.top} width={gap} height={chartH} fill="rgba(255,255,255,0.03)" />}
              <line x1={cx} x2={cx} y1={toY(c.high)} y2={toY(c.low)} stroke={clr} strokeWidth={1} />
              <rect x={cx - candleW / 2} y={bodyTop} width={candleW} height={bodyH}
                fill={clr} fillOpacity={isBull ? 0.85 : 1} stroke={clr} strokeWidth={0.5} />
            </g>
          );
        })}

        {tooltip && <line x1={toX(tooltip.idx)} x2={toX(tooltip.idx)} y1={PAD.top} y2={PAD.top + chartH}
          stroke="rgba(255,255,255,0.15)" strokeWidth={1} strokeDasharray="4 3" />}
      </svg>

      {tooltip && ttC && (
        <div style={{ position: 'absolute',
          left: Math.min(tooltip.x + 12, (svgRef.current?.clientWidth ?? 600) - 175),
          top:  Math.max(0, tooltip.y - 60),
          background: '#131829', border: '1px solid #1e2640', borderRadius: 10,
          padding: '10px 14px', fontFamily: 'DM Mono', fontSize: 12,
          pointerEvents: 'none', zIndex: 10, minWidth: 160,
          boxShadow: '0 8px 24px rgba(0,0,0,0.4)' }}>
          <div style={{ color: '#8892b0', marginBottom: 6, fontSize: 11 }}>{ttC.time}</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '3px 10px' }}>
            <span style={{ color: '#8892b0' }}>O</span><span>{fmtPrice(ttC.open)}</span>
            <span style={{ color: '#22c55e' }}>H</span><span style={{ color: '#22c55e' }}>{fmtPrice(ttC.high)}</span>
            <span style={{ color: '#ef4444' }}>L</span><span style={{ color: '#ef4444' }}>{fmtPrice(ttC.low)}</span>
            <span style={{ color: '#8892b0' }}>C</span>
            <span style={{ color: ttC.close >= ttC.open ? '#22c55e' : '#ef4444', fontWeight: 700 }}>{fmtPrice(ttC.close)}</span>
          </div>
          {ttC.volume != null && (
            <div style={{ marginTop: 5, color: '#8892b0', fontSize: 10 }}>
              Vol: <span style={{ color: 'var(--text-primary)' }}>{ttC.volume >= 1e6 ? (ttC.volume/1e6).toFixed(2)+'M' : ttC.volume.toFixed(0)}</span>
            </div>
          )}
        </div>
      )}
...
// ── SVG Line Chart for Crypto ─────────────────────────────
function CryptoSVGLine({ candles, height = 300, color }) {
  const [tooltip, setTooltip] = useState(null);
  const svgRef = useRef(null);
  if (!candles.length) return null;
  ...
  const area  = `M ${toX(0)},${toY(closes[0])} ` + candles.map((c, i) => `L ${toX(i)},${toY(c.close)}`).join(' ') +
    ` L ${toX(candles.length-1)},${PAD.top+chartH} L ${toX(0)},${PAD.top+chartH} Z`;

  function onMove(e) {
    if (!svgRef.current) return;
    const rect = svgRef.current.getBoundingClientRect();
    const mx   = (e.clientX - rect.left) / rect.width * chartW;
    const idx  = Math.max(0, Math.min(candles.length - 1, Math.round((mx / chartW) * (candles.length - 1))));
    setTooltip({ idx, x: e.clientX - rect.left, y: e.clientY - rect.top });
  }
  ...
  return (
    <div style={{ position: 'relative', width: '100%' }}>
      <svg ref={svgRef} viewBox={`0 0 ${W} ${height}`} width="100%" height={height}
        ...
        style={{ display: 'block', cursor: 'crosshair' }}>
        <defs>
          <linearGradient id={`cgrad_${color.replace('#','')}`} x1="0" y1="0" x2="0" y2="1">
        ...
        {candles.filter((_, i) => i % xStep === 0).map((c, i, arr) => {
          const origIdx = i * xStep;
          return <text key={i} x={toX(origIdx)} y={height-4} fill="#4a5578" fontSize={9} fontFamily="DM Mono" textAnchor="middle">{c.time}</text>;
        })}
        <path d={area} fill={`url(#cgrad_${color.replace('#','')})`} />
        <polyline points={pts} fill="none" stroke={color} strokeWidth={2} strokeLinejoin="round" />
        {tooltip && <line x1={toX(tooltip.idx)} x2={toX(tooltip.idx)} y1={PAD.top} y2={PAD.top+chartH} stroke="rgba(255,255,255,0.15)" strokeWidth={1} strokeDasharray="4 3" />}
        {ttC && <circle cx={toX(tooltip.idx)} cy={toY(ttC.close)} r={4} fill={color} />}
      </svg>
      {tooltip && ttC && (
        <div style={{ position: 'absolute',
          left: Math.min(tooltip.x + 12, (svgRef.current?.clientWidth ?? 600) - 160),
          top: Math.max(0, tooltip.y - 40),
          background: '#131829', border: '1px solid #1e2640', borderRadius: 8,
          padding: '8px 12px', fontFamily: 'DM Mono', fontSize: 12,
          pointerEvents: 'none', zIndex: 10 }}>
          <div style={{ color: '#8892b0', fontSize: 11, marginBottom: 3 }}>{ttC.time}</div>
  ...
  return (
    <div className="panel">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-muted)', display: 'flex', gap: 8, alignItems: 'center' }}>
              <span>Binance</span>
              {last?.close && <span style={{ fontFamily: 'DM Mono', fontWeight: 700, color: 'var(--text-primary)' }}>{fmtPrice(last.close)}</span>}
              {chgPct && <span style={{ color, fontWeight: 700 }}>{isUp ? '▲ +' : '▼ '}{Math.abs(chgPct)}%</span>}
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', borderRadius: 7, overflow: 'hidden', border: '1px solid var(--border)' }}>
            {[['candle','🕯 Candles'],['line','📈 Line']].map(([t, l]) => (
              <button key={t} onClick={() => setChartType(t)} style={{
                padding: '5px 11px', border: 'none', fontSize: 11, fontWeight: 600, cursor: 'pointer',
                background: chartType === t ? 'rgba(247,147,26,0.2)' : 'var(--bg-primary)',
                color:      chartType === t ? '#f7931a'               : 'var(--text-muted)',
              }}>{l}</button>
            ))}
          </div>
          {/* Intervals */}
          <div className="btn-group">
            {[['15m','15M'],['1h','1H'],['4h','4H'],['1d','1D'],['1w','1W']].map(([v, l]) => (
              <button key={v} className={`btn-range ${intv === v ? 'active' : ''}`} onClick={() => setIntv(v)}>{l}</button>
            ))}
          </div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <CryptoSVGCandles candles={candles} height={300} color={color} />
      ) : (
        <CryptoSVGLine candles={candles} height={300} color={color} />
      )}
      ...
      {last && !loading && (
        <div style={{ display: 'flex', gap: 20, padding: '8px 4px 0', borderTop: '1px solid var(--border)', fontSize: 12, fontFamily: 'DM Mono', flexWrap: 'wrap' }}>
          {[['O', last.open, 'var(--text-muted)'], ['H', last.high, '#22c55e'], ['L', last.low, '#ef4444'], ['C', last.close, color]].map(([label, val, clr]) => (
            <span key={label}>
              <span style={{ color: 'var(--text-muted)' }}>{label}: </span>
              <span style={{ color: clr, fontWeight: 700 }}>{fmtPrice(val)}</span>
            </span>
          ))}
        </div>
      )}
