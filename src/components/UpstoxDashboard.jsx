...
//       {/* SUB NAV */}
//       <div className="nav-tabs" style={{ marginBottom: 20 }}>
//         {[['overview','📊 Overview'],['analysis','🤖 AI Analysis'],['watchlist','⭐ Watchlist'],['gainers','📈 Gainers/Losers']].map(([key, label]) => (
//           <button key={key} className={`nav-tab ${activeTab === key ? 'active' : ''}`}
//             onClick={() => setActiveTab(key)}>{label}
//           </button>
//         ))}
...
// import {
//   useUpstoxIndices, useUpstoxStocks,
//   fetchUpstoxCandles, UPSTOX_INDICES, UPSTOX_STOCKS
// } from '../hooks/useUpstoxData';
...
//   return { macd: Math.round(m * 100) / 100, histogram: Math.round((m - s) * 100) / 100 };
// }
...
//   return { upper: mean + 2 * std, middle: mean, lower: mean - 2 * std, pct: Math.round(((last - (mean - 2 * std)) / (4 * std)) * 100) };
// }

// // ── Sub Components ─────────────────────────────────────────
// function IndexCard({ idx, active, onClick }) {
//   const up = (idx.chgPct ?? 0) >= 0;
//   return (
//     <div className={`index-card ${active ? 'active' : ''}`} onClick={onClick}>
//       <div className="index-label">{idx.label}</div>
//       {idx.price == null
...
//         : <div className="index-value">{fmt(idx.price)}</div>}
//       {idx.chgPct != null && (
//         <div className="index-change" style={{ color: up ? 'var(--green)' : 'var(--red)' }}>
//           {up ? '▲' : '▼'} {fmt(idx.change, 2)} ({Math.abs(idx.chgPct).toFixed(2)}%)
//         </div>
//       )}
//       {idx.price != null && (
//         <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 6 }}>
...
//   return (
//     <div className="mover-item" onClick={() => onAnalyze(s)}>
//       <div>
//         <div className="mover-sym">{s.sym}</div>
//         <div className="mover-name">{s.name}</div>
//         {s.volume && <div style={{ fontSize: 10, color: 'var(--text-muted)' }}>Vol: {(s.volume / 1e6).toFixed(2)}M</div>}
//       </div>
//       <div style={{ textAlign: 'right' }}>
//         <div className="mover-price">{s.price ? '₹' + fmt(s.price) : '---'}</div>
//         <ChgBadge val={s.chgPct} />
//         {s.high && <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 2 }}>H:{fmt(s.high)} L:{fmt(s.low)}</div>}
//       </div>
//     </div>
//   );
// }
...
//           {idx.label} <span style={{ color: 'var(--text-muted)', fontWeight: 400, fontSize: 13 }}>— Real-time Chart</span>
//           <span style={{ marginLeft: 8, fontSize: 10, background: 'rgba(201,168,76,0.15)', color: 'var(--gold)', padding: '2px 6px', borderRadius: 4 }}>UPSTOX LIVE</span>
//         </div>
//         <div className="btn-group">
//           {[['30minute','30M'],['day','1D'],['week','1W'],['month','1M']].map(([val, label]) => (
//             <button key={val} className={`btn-range ${interval === val ? 'active' : ''}`} onClick={() => setInterval2(val)}>{label}</button>
//           ))}
//         </div>
//       </div>
//       {loading
//         ? <div style={{ height: 300, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div className="loader" /></div>
//         : candles.length === 0
...
//                   formatter={v => ['₹' + fmt(v), '']}
//                 />
//                 <Area type="monotone" dataKey="close" stroke={color} strokeWidth={2} fill="url(#upGrad)" dot={false} activeDot={{ r: 4 }} />
//               </AreaChart>
//             </ResponsiveContainer>
//           )}
//     </div>
...
// // ── AI Analysis ─────────────────────────────────────────────
// function UpstoxAIAnalysis({ stocks, indices, preSelected }) {
//   const [selected, setSelected] = useState(preSelected?.sym ?? '');
//   const [customSym, setCustomSym] = useState('');
//   const [timeframe, setTimeframe] = useState('swing');
//   const [capital, setCapital] = useState(50000);
...
//   async function analyze() {
//     const sym = customSym.trim().toUpperCase() || selected;
//     if (!sym) { setError('Please select a stock.'); return; }
//     setError(''); setLoading(true); setResult(null);

//     const stockData = stocks.find(s => s.sym === sym);
//     const idxData = indices.find(i => i.label === sym);
//     const price = stockData?.price ?? idxData?.price;
//     const chgPct = stockData?.chgPct ?? idxData?.chgPct ?? 0;
//     const token = stockData?.token ?? idxData?.token;

//     if (!price) { setError('Price not loaded yet. Try again.'); setLoading(false); return; }

//     // Fetch real candles for technical analysis
//     let candleData = [];
//     if (token) {
//       candleData = await fetchUpstoxCandles(token, 'day');
//       setCandles(candleData);
//     }

//     const rsi = candleData.length > 15 ? calcRSI(candleData) : 50;
//     const macd = candleData.length > 26 ? calcMACD(candleData) : { histogram: 0 };
//     const bb = candleData.length > 20 ? calcBB(candleData) : { pct: 50 };
//     const shares = Math.floor(capital / price);

//     const prompt = `You are Market KA Khiladi, expert Indian stock market analyst. Help a BEGINNER investor.

// Stock: ${sym}
// Live Price (Upstox Real-time): ₹${price.toLocaleString('en-IN')}
// Today's Change: ${chgPct >= 0 ? '+' : ''}${chgPct.toFixed(2)}%
// RSI(14): ${rsi}
// MACD Histogram: ${macd.histogram}
// Bollinger Band %: ${bb.pct}%
// Timeframe: ${timeframe}
// Capital: ₹${capital.toLocaleString('en-IN')}
// Max Shares: ${shares}

// Give trade analysis using the REAL technical data above.
// Entry MUST be close to ₹${price.toFixed(2)}.
...
//   "rsiAnalysis": "What RSI ${rsi} means for this stock",
//   "macdAnalysis": "What MACD histogram ${macd.histogram} means",
//   "analysis": "2-3 sentences in simple English for a beginner",
//   "risks": ["Risk 1", "Risk 2", "Risk 3"]
// }`;
...
//       </div>
//       <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 16 }}>
//         Uses real-time Upstox prices + actual OHLC candles for technical analysis.
//       </p>
...
//           <option value="investment">Investment (3–6 months)</option>
//         </select>
//         <input className="ai-input" type="number" placeholder="Capital (₹)" value={capital} onChange={e => setCapital(Number(e.target.value))} />
//         <button className="btn-analyze" onClick={analyze} disabled={loading}>{loading ? '⏳ Analyzing...' : '🔍 Analyze'}</button>
//       </div>
//       {error && <p style={{ color: 'var(--red)', fontSize: 13, margin: '12px 0' }}>⚠️ {error}</p>}
//       {loading && <div className="ai-result"><div className="loader-wrap"><div className="loader" /><div className="loader-text">Fetching live data & generating signal...</div></div></div>}
//       {result && !loading && (
//         <div className="ai-result fade-in">
//           <div className={`signal-banner ${sigClass}`}>
//             <div>
//               <div style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 4 }}>Signal</div>
//               <div className="signal-word">{result.signal === 'BUY' ? '🟢' : result.signal === 'SELL' ? '🔴' : '🟡'} {result.signal}</div>
//             </div>
//             <div className="signal-meta">
//               <div className="signal-stock">{customSym || selected}</div>
//               <div className="signal-info">{result.shares} shares · ₹{Number(result.capitalUsed).toLocaleString('en-IN')} used</div>
//             </div>
//             <div className="confidence"><div className="confidence-label">Confidence</div><div className="confidence-value">{result.confidence}%</div></div>
//           </div>
//           <div className="trade-levels">
...
//               <div style={{ fontSize: 10, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 4 }}>RSI Analysis</div>
//               <div style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.5 }}>{result.rsiAnalysis}</div>
//             </div>
//             <div style={{ background: 'var(--bg-secondary)', borderRadius: 8, padding: 12 }}>
//               <div style={{ fontSize: 10, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 4 }}>MACD Analysis</div>
//               <div style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.5 }}>{result.macdAnalysis}</div>
//             </div>
//           </div>
//           <div className="analysis-text">{result.analysis}</div>
//           <div className="risk-box">
...
//   const sorted   = [...stocks].filter(s => s.chgPct != null).sort((a, b) => b.chgPct - a.chgPct);
//   const gainers  = sorted.slice(0, 5);
//   const losers   = [...sorted].reverse().slice(0, 5);
...
//     <div>
//       {/* UPSTOX BANNER */}
//       <div style={{ background: 'linear-gradient(135deg,rgba(59,130,246,0.1),rgba(10,14,26,0))', border: '1px solid rgba(59,130,246,0.2)', borderRadius: 12, padding: '12px 20px', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 12 }}>
//         <span style={{ fontSize: 24 }}>📡</span>
//         <div>
//           <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--blue)' }}>Upstox Live Data</div>
//           <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>Real-time NSE/BSE prices · Refreshes every 30 seconds · Official API</div>
//         </div>
//         <div style={{ marginLeft: 'auto', fontSize: 11, color: 'var(--green)', display: 'flex', alignItems: 'center', gap: 6 }}>
//           <div style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--green)', animation: 'pulse 2s infinite' }} />
//           Connected
//         </div>
//       </div>
...
//       {/* NAV */}
//       <div className="nav-tabs" style={{ marginBottom: 20, background: 'none', borderBottom: '1px solid var(--border)', paddingLeft: 0 }}>
//         {[['overview','📊 Overview'],['analysis','🤖 AI Analysis'],['watchlist','⭐ Watchlist'],['gainers','📈 Gainers/Losers']].map(([key, label]) => (
...
//         <div className="fade-in">
//           <div className="index-grid">
//             {indices.map((idx, i) => <IndexCard key={idx.label} idx={idx} active={activeIdx === i} onClick={() => setActiveIdx(i)} />)}
//           </div>
//           <div className="dashboard-grid">
//             <ChartPanel activeIdx={activeIdx} />
//             <div className="panel" style={{ overflowY: 'auto', maxHeight: 430 }}>
//               <div className="panel-header"><div className="panel-title">Top Movers</div><div className="panel-sub">Real-time</div></div>
//               {[...stocks].filter(s => s.chgPct != null).sort((a, b) => Math.abs(b.chgPct) - Math.abs(a.chgPct)).slice(0, 8).map(s => (
//                 <StockRow key={s.sym} s={s} onAnalyze={handleAnalyze} />
//               ))}
//             </div>
//           </div>
//         </div>
...
//       {activeTab === 'analysis' && (
//         <div className="fade-in">
//           <UpstoxAIAnalysis stocks={stocks} indices={indices} preSelected={aiStock} />
//         </div>
//       )}
...
//       {activeTab === 'watchlist' && (
//         <div className="fade-in panel">
//           <div className="panel-header"><div className="panel-title">⭐ Watchlist — Live Prices</div></div>
//           <div className="search-bar">
//             <span>🔍</span>
//             <input placeholder="Search stocks..." value={search} onChange={e => setSearch(e.target.value)} />
//           </div>
//           <table className="data-table">
//             <thead><tr><th>Symbol</th><th>Company</th><th>Price</th><th>Change</th><th>High</th><th>Low</th><th>Volume</th><th>Action</th></tr></thead>
//             <tbody>
//               {filtered.map(s => (
//                 <tr key={s.sym}>
//                   <td>{s.sym}</td>
...
//                   <td style={{ color: (s.chgPct ?? 0) >= 0 ? 'var(--green)' : 'var(--red)' }}>{s.chgPct != null ? (s.chgPct >= 0 ? '+' : '') + s.chgPct.toFixed(2) + '%' : '---'}</td>
//                   <td style={{ color: 'var(--green)' }}>{s.high ? '₹' + fmt(s.high) : '---'}</td>
//                   <td style={{ color: 'var(--red)' }}>{s.low ? '₹' + fmt(s.low) : '---'}</td>
//                   <td>{s.volume ? (s.volume / 1e6).toFixed(2) + 'M' : '---'}</td>
//                   <td><button onClick={() => handleAnalyze(s)} style={{ background: 'var(--gold)', color: '#000', border: 'none', borderRadius: 5, padding: '4px 10px', fontSize: 11, fontWeight: 600, cursor: 'pointer' }}>Analyze</button></td>
//                 </tr>
//               ))}
//             </tbody>
//           </table>
...
//             <table className="data-table">
//               <thead><tr><th>Stock</th><th>Price</th><th>Change</th><th>High</th><th>Volume</th></tr></thead>
//               <tbody>
//                 {gainers.map(s => (
//                   <tr key={s.sym} style={{ cursor: 'pointer' }} onClick={() => handleAnalyze(s)}>
//                     <td>{s.sym}</td>
//                     <td>₹{fmt(s.price)}</td>
//                     <td style={{ color: 'var(--green)' }}>+{s.chgPct?.toFixed(2)}%</td>
//                     <td>₹{fmt(s.high)}</td>
//                     <td>{s.volume ? (s.volume / 1e6).toFixed(2) + 'M' : '---'}</td>
//                   </tr>
//                 ))}
...
//             <table className="data-table">
//               <thead><tr><th>Stock</th><th>Price</th><th>Change</th><th>Low</th><th>Volume</th></tr></thead>
//               <tbody>
//                 {losers.map(s => (
//                   <tr key={s.sym} style={{ cursor: 'pointer' }} onClick={() => handleAnalyze(s)}>
//                     <td>{s.sym}</td>
//                     <td>₹{fmt(s.price)}</td>
//                     <td style={{ color: 'var(--red)' }}>{s.chgPct?.toFixed(2)}%</td>
//                     <td>₹{fmt(s.low)}</td>
//                     <td>{s.volume ? (s.volume / 1e6).toFixed(2) + 'M' : '---'}</td>
//                   </tr>
//                 ))}
